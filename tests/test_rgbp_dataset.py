...
def mock_datasets(temp_dir):
    ...
    datasets_info = [
        ('SCRREAM', 'single_file_clock', ['scene01', 'scene09_full_00', 'scene09_reduced_00']),
...
class TestDatasetCreation:
    ...
    def test_specific_dataset_creation(self, test_config, mock_datasets, dataset_name):
        ...
        with patch('builtins.print'):
            datasets = create_datasets_from_config(test_config, dataset_names=[dataset_name])
...
class TestDatasetFunctionality:
    ...
    @pytest.mark.parametrize("pol_format", ['single_file_clock', 'separate_files', 'mosaic'])
    def test_polarization_format_validation(self, temp_dir, mock_datasets, pol_format):
        ...
        dataset = RGBP_Dataset(
            ...
            polarization_format=pol_format,
            few_images=True
        )
    ...
    def test_dataset_length_and_indexing(self, temp_dir, mock_datasets):
        ...
        if len(dataset) > 0:
            ...
            expected_keys = ['rgb', 'specular', 'diffuse', 'intrinsics', 'DoLP', 'AoP', 'f_spec']
...
class TestDataLoading:
    ...
    @pytest.mark.parametrize("resize_mode,target_size", [
        ('crop', (224, 224)),
        ('resize', (224, 224)),
        ('pad', (224, 224))
    ])
