...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate CHOLEC80 directory structure conversion"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=80,
        help="Maximum number of version directories (default: 80)",
    )
    parser.add_argument(
        "--version-prefix",
        default="v",
    ...
    print("-" * 80)
    print(
        f"{'Source Dir':<12} {'Dest Version':<12} {'Source JPGs':<12} {'Dest JPGs':<12} {'Dest JSONs':<12} {'Status':<10}"
    )
    print("-" * 80)
    ...
    for subdir in src_subdirs:
        ...
        if verbose:
            ...
            print(
                f"{subdir:<12} {version:<12} {src_jpg:<12} {'N/A':<12} {'N/A':<12} {status:<10}"
            )
    ...
    for version, subdirs in src_by_version.items():
        ...
        src_dirs_str = (
            ", ".join(subdirs) if len(subdirs) <= 3 else f"{len(subdirs)} directories"
        )

        print(
            f"{'N/A':<12} {version:<12} {expected_jpg_count:<12} {actual_jpg_count:<12} {actual_json_count:<12} {status:<10}"
        )
    ...
    print("-" * 80)
    print(
        f"{'TOTAL':<12} {'N/A':<12} {total_src_jpg:<12} {total_dest_jpg:<12} {total_dest_json:<12} {'VALID' if all_valid else 'INVALID':<10}"
    )
    print("-" * 80)
