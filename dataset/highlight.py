...
class HighlightDataset(Mono3D_Dataset):
    ...
    def _wrap_existing_dataset(self, dataset_instance: Mono3D_Dataset) -> None:
        ...
        attributes_to_copy = [
            ...
            'rgbpathlist', 'pathlist', 'depthpathlist', 'poseslist', 'intrinsicslist', 
            'distortionslist', 'Tlist', 'Tinvlist', 'sampler',
            
            # Configuration attributes
            'numframes', 'numvideos', 'name', 'fps', 'frameskip', 'frameskip_set',
            'frameskip_set_curriculum', 'frameskip_curriculum_step', 'manual_frameskip',
            
            # Dimension and transformation attributes
            'height', 'width', 'original_height', 'original_width', 'aspect_ratio',
            'backbone_height', 'backbone_width', 'resize_transform',
            
            # Feature flags
            'as_euler', 'as_embedding', 'unit_translation', 'as_quat',
            'with_paths', 'with_frameskip', 'with_intrinsics', 'with_distortions',
            'with_fundamental', 'with_global_poses', 'with_depth',
            'transforms_only', 'target_pose_only', 'random_pose', 'random_pose_ranges',
            
            # Augmentation settings
            'color_augmentation_prob', 'geometric_augmentation_prob',
            'reverse_augmentation_prob', 'standstill_augmentation_prob',
            
            # Learning parameters
            'standardize', 'target_length', 'curriculum_factor',
            
            # Storage and caching
            'DEVICE', 'is_gcs', 'preload_in_memory', 'preload_transforms',
            'frame_cache', 'depth_cache', 'embedding_cache',
            
            # Other attributes
            'excluded', 'order_check'
        ]
    ...
    def _create_from_class(self, base_dataset_class: type, kwargs: Dict[str, Any]) -> None:
        """
    ...
    def _find_rectangle_with_least_highlights(self, binary_mask: torch.Tensor, rect_size: Optional[Tuple[int, int]]) -> Tuple[int, int, int, int]:
        """
        ...
        tops = torch.arange(max_top, device=binary_mask.device)[:, None]  # Shape: (max_top, 1)
        lefts = torch.arange(max_left, device=binary_mask.device)[None, :]  # Shape: (1, max_left)
    ...
    def _compute_rectangles_for_framestack(self, framestack):
        ...
        for i in range(framestack.shape[0]):
            ...
            highlight_mask = highlight_mask.squeeze(0)  # Remove channel dimension: (H, W)
    ...
    def __getitem__(self, idx):
        ...
        if self.return_mask and 'framestack' in output:
            ...
            output['highlight_coverage'] = torch.tensor(highlight_coverage, dtype=torch.float32)
        ...
        if self.rect_size is not None and 'framestack' in output:
            ...
            if self.return_rect:
                cropped_frames, cropped_masks = self._crop_rectangles_from_framestack(framestack, rectangles)
    ...
    def _crop_rectangle_from_frame(self, frame, rect_coords):
        ...
        if top == 0 and left == 0 and bottom == 0 and right == 0:
            ...
            if self.rect_size is not None:
                ...
                empty_frame = torch.zeros(frame.shape[0], target_height, target_width, device=frame.device, dtype=frame.dtype)
                empty_mask = torch.zeros(1, target_height, target_width, device=frame.device, dtype=frame.dtype)
                return empty_frame, empty_mask
            else:
                return frame, torch.zeros(1, frame.shape[1], frame.shape[2], device=frame.device, dtype=frame.dtype)
    ...
    def _crop_rectangles_from_framestack(self, framestack, rectangles):
        ...
        for i in range(framestack.shape[0]):
            ...
            cropped_frame, cropped_mask = self._crop_rectangle_from_frame(frame, rect_coords)
...
def draw_rect(
    ...
    for t in range(thickness):
        ...
        if bottom_border < H and bottom_border != top_border and left_border <= right_border:
            for c in range(C):
                output[c, bottom_border, left_border:right_border+1] = color_values[c]
        ...
        if right_border < W and right_border != left_border and top_border <= bottom_border:
            for c in range(C):
                output[c, top_border:bottom_border+1, right_border] = color_values[c]
    ...
    if isinstance(color, str):
        ...
        if len(color) >= 3:
            ...
            if all(isinstance(x, int) and 0 <= x <= 255 for x in rgb):
                ...
                raise ValueError(f"RGB values must be in range [0, 255] or [0, 1]: {rgb}")
        else:
