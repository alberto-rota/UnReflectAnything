"""
Augmentation functions for the dataset module.
"""
...
def color_augmentation(
    ...
    Args:
        framestack: A tensor representing a stack of images of shape (B, C, H, W) or (2, C, H, W)
        camera_pose: A list representing transformation parameters, not modified by this function
        p: Probability of applying sharpness adjustments. Default is 0.2
...
def geometric_augmentation(
    framestack: torch.Tensor,
    camera_pose: List[float],
    depthstack: Optional[torch.Tensor] = None,
    p: float = 0.2,
) -> Tuple[torch.Tensor, List[float]]:
    """
    Applies random geometric augmentations to a tensor of images, modifying the transformation vector accordingly, with
    a set probability.

    Args:
        framestack: A tensor representing a stack of images of shape (B, C, H, W) or (2, C, H, W)
        camera_pose: A list representing transformation parameters, modified based on applied augmentations
        depthstack: Optional depth tensor to apply the same geometric transformations to
        p: Probability of applying each geometric augmentation. Default is 0.2

    Returns:
        A tuple containing the augmented image tensor and the adjusted transformation list
    """
