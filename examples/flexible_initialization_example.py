...
def example_3_mixed_instances_and_configs():
    ...
    pol_encoder = POLViTEncoder(
        {"in_ch": 3, "embed_dim": 768, "depth": 4, "n_heads": 12, "patch_size": 16}
    )
    ...
    model = RGBPOLDecomposer(
        ...
        specular_decoder_or_config={
            "use_bn": True,
            "fusion_hidden_size": 512,
        },  # Config dict
        diffuse_decoder_or_config={
            "use_bn": False,
            "fusion_hidden_size": 256,
        },  # Config dict
...
def test_forward_pass(model):
    ...
    batch = {
        ...
        "AoP": torch.randn(batch_size, 1, height, width, device="cuda")
        * 3.14159,  # [0, π)
        "DoP": torch.rand(batch_size, 1, height, width, device="cuda"),  # [0, 1]
    }

    print(
        f"Input shapes: RGB {batch['rgb'].shape}, AoP {batch['AoP'].shape}, DoP {batch['DoP'].shape}"
    )
    ...
    print(
        f"  Token shapes: RGB {outputs['tokens']['rgb'].shape}, POL {outputs['tokens']['pol'].shape}, Fused {outputs['tokens']['fused'].shape}"
    )
